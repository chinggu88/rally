#!/usr/bin/env bash
# Local runner for BWF HSBC World Tour calendar crawl job.
# Usage: ./batch/run-bwf-calendar.sh [--year YYYY] [--dry-run]
# Examples:
#   ./batch/run-bwf-calendar.sh                # current year
#   ./batch/run-bwf-calendar.sh --year 2027    # specific year
#   ./batch/run-bwf-calendar.sh --dry-run      # print only, no DB writes
...
log() { printf "\033[36m[run-bwf-calendar]\033[0m %s\n" "$*"; }
die() { printf "\033[31m[run-bwf-calendar]\033[0m %s\n" "$*" >&2; exit 1; }
...
cd "$ROOT_DIR"
log "Running batch.jobs.bwf_calendar.main $*"
PYTHONPATH=. python -u -m batch.jobs.bwf_calendar.main "$@"
